            ...
            if (!booking.razorpayPaymentId) {
              ...
              try {
                ...
                if (
                  booking.schema
                    .path("paymentStatus")
                    .enumValues.includes("refund_failed")
                ) {
                  booking.paymentStatus = "refund_failed";
                } else {
...
module.exports.createBooking = async (req, res) => {
  try {
    ...
    if (req.user.userType === "student") {
      ...
      discountAmount = parseFloat(subtotal) * 0.2;
    }
...
module.exports.verifyPayment = async (req, res) => {
  try {
    ...
    try {
      ...
      const userUpdate = User.findByIdAndUpdate(booking.user._id, {
        $push: { bookings: booking._id }
      });

      // Update listing's bookings
      const listingUpdate = Listing.findByIdAndUpdate(booking.listing._id, {
        $push: { bookings: booking._id }
      });
